...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(!interaction.member.permissions.has('KickMembers')) //KickMembers
            errorArray.push('You do not have the required permissions for this action.')

        if(!member) return interaction.reply({
            embeds: [Embed.setDescription("This user is no longer a member of this guild.")]})
        ...
        switch(splitArray[1]) {
            ...
            case 'Ban' : {
                member.kick(`Banned by: ${interaction.user.tag} | Member Logging System`)
                .then(() => {
